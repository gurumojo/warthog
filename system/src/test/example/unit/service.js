...
describe('service', function(){
	...
	beforeEach(module('gurumojo.service'));
	...
	describe('meta', function(){
		...
		it('should return title string', inject(function(meta){
			...
			expect(meta.title).toEqual('ng-app');
		}));
	...
	describe('crypto', function(){
		...
		beforeEach(inject(function($injector){
			...
			$httpBackend.when('GET', 'lib/crypt/md5.js').respond(200,
		...
		it('should return an object with hash methods', function(){
			...
			$httpBackend.expectGET('lib/crypt/md5.js');
	...
	describe('parser', function(){
		...
		beforeEach(inject(function($injector){
			$httpBackend = $injector.get('$httpBackend');
			$httpBackend.when('GET', 'lib/x2js/xml2json.js').respond(200,
		...
		it('should return an object with hash methods', function(){
			...
			$httpBackend.expectGET('lib/x2js/xml2json.js');
	...
	describe('session', function(){
		...
		beforeEach(inject(function($injector){
			...
			$httpBackend.when('GET', 'json/user/login').respond(200,
				'{"user":{"hash":"e17fd240178a67ce516bc6fdbb716c92"}}'
			);
			$httpBackend.when('POST', 'json/user/login').respond(201,
				'{"user":{"email":"test@ng.io"}}'
			);
			$httpBackend.when('GET', 'json/user/logout').respond(200,
				'{"user":{"logout":"test"}}'
			);
		}));
		...
		it('should logout on request', function(){
			...
			$httpBackend.expectGET('json/user/logout');
			inject(function(session){
				expect(session.user).toBeUndefined();
				session.logout();
				$httpBackend.flush();
